...
select ename as 사원명, sal as 월급여, (sal * 12 + nullif(comm, 0)) as 연봉 from emp;
...
select ename, empno from emp where hiredate = TO_DATE('1981-09-28', 'YYYY-MM-DD');
--select ename, empno from emp where hiredate = TO_DATE('81-09-28', 'YYYY-MM-DD');
select ename, empno from emp where hiredate = TO_DATE('1981/09/28', 'YYYY-MM-DD');
select ename, empno from emp where hiredate = TO_DATE('28-9-1981', 'DD-MM-YYYY');
--select ename, empno from emp where hiredate = TO_DATE('81/09/28', 'YYYY-MM-DD');

-- 27. 날짜 타입의 범위를 기준으로 검색
-- 범위비교시 연산자 : between~and 1980-12-17~1981-09-28
select empno, ename, hiredate from emp where hiredate between TO_DATE('1980-12-17', 'YYYY-MM-DD') and TO_DATE('1981-09-28', 'YYYY-MM-DD');
